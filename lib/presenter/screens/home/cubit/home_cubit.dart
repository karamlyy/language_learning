...
class HomeCubit extends Cubit<BaseState> {
  ...
  LanguagePairModel? selectedLanguagePair;
  ...
  void getAllLanguagePairs() async {
    ...
    result.fold(
          ...
          (data) {
        languagePairs = data;
        selectedLanguagePair = languagePairs.firstWhere((pair) => pair.isSelected);
        emit(SuccessState(data: data));
      },
    );
  }

  void setSelectedLanguagePair(LanguagePairModel pair) {
    selectedLanguagePair = pair;
    emit(SuccessState(data: {"categories": categories, "words": words, "selectedPair": pair}));
  }
